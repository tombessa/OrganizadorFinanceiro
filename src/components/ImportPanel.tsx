...
export default function ImportPanel({ accessToken }: ImportPanelProps) {
  ...
  const [documentStatus, setDocumentStatus] = useState<"POSTED" | "PROJECTED">("POSTED");
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (adapter === "INTER_CARD_CSV") form.set("documentStatus", documentStatus);
      form.set("file", file);
  ...
  return (
    <section className="workspace import-workspace" aria-labelledby="import-title">
      ...
      <p className="section-description">Extratos e faturas Inter, além das faturas Itaú em XLSX, são extraídos com idempotência. Os demais formatos ficam armazenados com segurança enquanto seus adaptadores são habilitados.</p>
      <form className="import-form" onSubmit={submit}>
        ...
        {adapter === "INTER_CARD_CSV" ? <div><label htmlFor="document-status">Situação da fatura</label><select id="document-status" value={documentStatus} onChange={(event) => setDocumentStatus(event.target.value as "POSTED" | "PROJECTED")}><option value="POSTED">Efetiva — lançamentos já realizados</option><option value="PROJECTED">Projetada — fatura futura</option></select></div> : null}
        {adapter === "ITAU_CARD_XLSX" ? <div className="import-note"><strong>Situação automática</strong><small>O sistema identifica no Excel se a fatura está paga, aberta ou futura.</small></div> : null}
