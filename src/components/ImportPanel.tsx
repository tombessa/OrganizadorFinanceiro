...
export default function ImportPanel({ accessToken }: ImportPanelProps) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (targetType === "CARD") form.set("documentStatus", documentStatus);
      form.set("file", file);
  ...
  return (
    <section className="workspace import-workspace" aria-labelledby="import-title">
      ...
      <p className="section-description">Extratos e faturas Inter já são extraídos com idempotência. Os demais formatos ficam armazenados com segurança enquanto seus adaptadores são habilitados.</p>
      <form className="import-form" onSubmit={submit}>
        ...
        {targetType === "CARD" ? <div><label htmlFor="document-status">Situação da fatura</label><select id="document-status" value={documentStatus} onChange={(event) => setDocumentStatus(event.target.value as "POSTED" | "PROJECTED")}><option value="POSTED">Efetiva — lançamentos já realizados</option><option value="PROJECTED">Projetada — fatura futura</option></select></div> : null}
